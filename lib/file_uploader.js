    ...
    if (err) {
      ...
      mediaFile.on('data', function(chunk) {
        ...
        self._appendMedia(mediaTmpId, chunk.toString('base64'), chunkNumber, function(err, bodyObj, resp) {
          ...
          if (err) {
            cb(err);
          } else if (self._isUploadComplete()) {
...
FileUploader.prototype._isUploadComplete = function() {
   ...
   * @param  {Function} cb
   */
FileUploader.prototype._finalizeMedia = function(mediaId, cb) {
  var self = this;
   ...
   *
   * @param  {String}   media_id        mediaIdStr received from Twitter after sending INIT comand.
   * @param  {String}   chunk_part      Base64-encoded String chunk of the media file.
   * @param  {Number}   segmentIndex   Index of the segment.
   * @param  {Function} cb
   */
FileUploader.prototype._appendMedia = function(mediaIdStr, chunkPart, segmentIndex, cb) {
  var self = this;
  self._twit.post('media/upload', {
    command: 'APPEND',
    media_id: mediaIdStr.toString(),
