...
var StreamingAPIConnection = function(reqOpts, twittyOptions) {
  this.reqOpts = reqOpts;
  this.twittyOptions = twittyOptions;
  ...
  self.request.on('response', function(response) {
    ...
    if (STATUS_CODES_TO_ABORT_ON.indexOf(self.response.statusCode) !== -1) {
      ...
      gunzip.on('end', function() {
        ...
        var error = helpers.makeTwittyError('Bad Twitter streaming request: ' + self.response.statusCode);
      ...
      gunzip.on('error', function(err) {
        ...
        var twittyErr = helpers.makeTwittyError(errMsg);
        twittyErr.statusCode = self.response.statusCode;
        helpers.attachBodyInfoToError(twittyErr, compressedBody);
        self.emit('parser-error', twittyErr);
      });
...
StreamingAPIConnection.prototype._scheduleReconnect = function() {
  ...
  } else if (!self._usedFirstReconnect) {
...
StreamingAPIConnection.prototype._setOauthTimestamp = function() {
  ...
  if (self.reqOpts.oauth) {
    ...
    self.reqOpts.oauth.timestamp = Math.floor(oauthTs / 1000).toString();
  }
};
