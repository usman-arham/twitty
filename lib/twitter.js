...
//  Twitty
//
var Twitty = function(config) {
  if (!(this instanceof Twitty)) {
    return new Twitty(config);
  }
  ...
  this.config = config;
  this._twitterTimeDiff = 0;
};

Twitty.prototype.get = function(path, params, callback) {
  return this.request('GET', path, params, callback);
};

Twitty.prototype.post = function(path, params, callback) {
  return this.request('POST', path, params, callback);
};

Twitty.prototype.delete = function(path, params, callback) {
  return this.request('DELETE', path, params, callback);
};

Twitty.prototype.request = function(method, path, params, callback) {
  var self = this;
        ...
        self._doRestApiRequest(reqOpts, twitOptions, method, function(err, parsedBody, resp) {
          ...
          if (self.config.trusted_cert_fingerprints) {
            if (!resp.socket.authorized) {
              ...
              var err = helpers.makeTwittyError('The peer certificate was not signed; ' + authErrMsg);
              return _returnErrorToUser(err);
            }
 ...
 */
Twitty.prototype.postMediaChunked = function(params, cb) {
  var self = this;
  try {
...
Twitty.prototype._updateClockOffsetFromResponse = function(resp) {
  var self = this;
  if (resp && resp.headers && resp.headers.date &&
    ...
    self._twitterTimeDiff = twitterTimeMs - Date.now();
  }
};
 ...
 */
Twitty.prototype._buildReqOpts = function(method, path, params, isStreaming, callback) {
  var self = this;
  if (!params) {
    params = {};
  }
  ...
  if (isStreaming) {
    ...
    var streamEndpointMap = {
      user: endpoints.USER_STREAM,
      site: endpoints.SITE_STREAM
    };
    var endpoint = streamEndpointMap[path] || endpoints.PUB_STREAM;
    ...
    if (FORMDATA_PATHS.indexOf(path) > -1 && (method === 'POST' || method === 'PUT')) {
      ...
      // of the params
      finalParams = {};
    } else if (JSONPAYLOAD_PATHS.indexOf(path) > -1 && (method === 'POST' || method === 'PUT')) {
  ...
  if (!self.config.app_only_auth) {
    ...
    var oauthTs = Date.now() + self._twitterTimeDiff;
 ...
 * @return {Undefined}
 */
Twitty.prototype._doRestApiRequest = function(reqOpts, twitOptions, method, callback) {
  var requestMethod = request[method.toLowerCase()];
  var req = requestMethod(reqOpts);
  ...
  var onRequestComplete = function() {
    if (body !== '') {
      try {
        ...
        var err = helpers.makeTwittyError('JSON decode error: Twitter HTTP response body was not valid JSON');
    ...
    if (typeof body === 'object' && (body.error || body.errors)) {
      ...
      var err = helpers.makeTwittyError('Twitter API Error');
      err.statusCode = response ? response.statusCode : null;
 ...
 */
Twitty.prototype.stream = function(path, params) {
  var self = this;
  var twitOptions = (params && params.twit_options) || {};
 ...
 * @return {Undefined}
 */
Twitty.prototype._getBearerToken = function(callback) {
  var self = this;
  ...
  function(err, bearerToken) {
    if (err) {
      // return the fully-qualified Twitty Error object to caller
      callback(err, null);
      return;
    }
...
Twitty.prototype.normalizeParams = function(params) {
...
Twitty.prototype.setAuth = function(auth) {
  var self = this;
  var configKeys = [
    'consumer_key',
    'consumer_secret',
...
Twitty.prototype.getAuth = function() {
  return this.config;
};
...
//
Twitty.prototype._validateConfigOrThrow = function(config) {
  ...
  if (typeof config.timeout_ms !== 'undefined' && isNaN(Number(config.timeout_ms))) {
    throw new TypeError('Twitty config `timeout_ms` must be a Number. Got: ' + config.timeout_ms + '.');
  }

  if (config.app_only_auth) {
    var authType = 'app-only auth';
    var requiredKeys = REQUIRED_FOR_APP_AUTH;
  } else {
    var authType = 'user auth';
    var requiredKeys = REQUIRED_FOR_USER_AUTH;
  }

  requiredKeys.forEach(function(reqKey) {
    if (!config[reqKey]) {
      var errMsg = util.format('Twitty config must include `%s` when using %s.', reqKey, authType);
      throw new Error(errMsg);
    }
  });
};

module.exports = Twitty;