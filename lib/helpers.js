...
exports.makeTwittyError = function(message) {
  var err = new Error();
  if (message) {
    err.message = message;
  }
 ...
 */
exports.getBearerToken = function(consumerKey, consumerSecret, cb) {
  // use OAuth 2 for app-only auth (Twitter requires this)
  // get a bearer token using our app's credentials
  var b64Credentials = new Buffer(consumerKey + ':' + consumerSecret).toString('base64');
  request.post({
    ...
    if (err) {
      var error = exports.makeTwittyError(err.toString());
      exports.attachBodyInfoToError(error, body);
      return cb(error, body, res);
    }
    
    if ( !body ) {
      var error = exports.makeTwittyError('Not valid reply from Twitter upon obtaining bearer token');
      exports.attachBodyInfoToError(error, body);
      return cb(error, body, res);
    }

    if (body.token_type !== 'bearer') {
      var error = exports.makeTwittyError('Unexpected reply from Twitter upon obtaining bearer token');
      exports.attachBodyInfoToError(error, body);
      return cb(error, body, res);
    }
